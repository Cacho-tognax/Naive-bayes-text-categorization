...
training_test_sizes = [640]  # , 128, 256, 512]
...
def evaluate_pre_split(train_count_matrix, train_category, test_count_matrix, test_category, categories_count):
    ...
    for size in training_test_sizes:
        ...
        acc_results = []
        for i in range(30):
            skf = StratifiedKFold(n_splits=int(np.floor(len(train_category)/size)), shuffle=True)
            z_train = []
            for test, train, in skf.split(train_count_matrix, train_category):
                z_train = train
            print('training test size offset = ' + str(abs(len(z_train) - size)))
            x_train = train_count_matrix[z_train]
            y_train = train_category[z_train]
            model = MultinomialNB()
            model.fit(x_train, y_train)
            classification = model.predict(test_count_matrix)
            probability_classification = model.predict_proba(test_count_matrix)
            acc_results.append(np.mean(classification == test_category) * 100)  # for percentages
            # now work only for roc
            y_true = []
            for j in test_category:
                row = [0] * categories_count
                row[j] = 1
                y_true.append(row)
            y_score = np.asarray(probability_classification)
